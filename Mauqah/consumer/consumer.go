...
func main() {
	...
	go func() {
		...
		for d := range messageChannel {
			...
			if addTask.Response == "" {
				log.Printf("Message Recieved : Name %s  Age %d  Operation  %s", addTask.Name, addTask.Age, addTask.Operation)
			} else {
				log.Printf("Message Recieved : Record id %d  %s ", addTask.RecordId, addTask.Response)
			}
